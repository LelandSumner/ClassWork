...
\section*{Starting and stopping \emx}
...
in our listing was \code{setColor} though we typed
...
\section*{Make the \texttt{NewtonsPear} game}

Using your current running \emax program,
you will create a new Java program
that is almost identical to your \verb\NewtonsPear.java program
but with a different color for the ``apple''.

Choose \verb\File->Save Buffer As...\ and,
in the ``minibuffer'' at the bottom of your \emax window,
type in \verb\NewtonsPear.java\
at the end of the line that says \verb\Write file:\.  
This will create a new file in your \verb\Lab01\ directory,
and the file will be named \verb\NewtonsPear.java\, as you would expect.
(Go to your shell window and run the \verb\ls\ command
to see that the new file is there.)

All we need to do is to change all occurrences of ``apple'' to ``pear'',
and to change the color of the ``pear'' to yellow.
Well, there is one additional thing you need to do:
change the \verb\class\ name to \verb\NewtonsPear\.

Here is what you do:

\begin{itemize}
\item
  Change all occurrences of \verb\Apple\ with the word \verb\Pear\.
  In \emax, choose \verb\Edit->Search->Replace...\,
  and in the minibuffer enter \verb\Apple<Enter>Pear<Enter>\
  where \verb\<Enter>\ means to hit the Enter key on the keyboard.
  \emax will query you for each change -- hit the spacebar if you want
  to make the change (and you should).
\item
  Similarly, replace all occurrences of \verb\apple\ with the word \verb\pear\.
  There will be many of these, and you'll get tired of hitting the
  spacebar, so if you hit the \verb\a\ key, \emax will make all the
  remainig replacements without asking.
\item Finally, lets fix up the line that determines the color of the pear: 
  change the color of the oval from \verb\red\ to \verb\yellow\.
\end{itemize}

\subsection*{Checkpoint 7}
Show us your changes and your running program.
