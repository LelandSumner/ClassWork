...
public class Pinball 
  ...
  private Location2D norm(Location2D pt) {
    return new Location2D(-pt.getY(), pt.getX());
  }

  private Location2D difference(Location2D pt1, Location2D pt2) {
    return new Location2D(pt1.getX() - pt2.getX(), pt1.getY() - pt2.getY());
  }
  ...
  public void bounceOffOf(Sprite other) {
    if (other instanceof OvalSprite) {
      ...
      Location2D normalVector = norm(centerVector);
      ...
      Location2D tangentVelocity = difference(velocity, normalVelocity);
    
      // reflect (invert) the normal component and add to unchanged tangent component.
      setVelocity(difference(tangentVelocity, normalVelocity));
